        ...
        if (command.startsWith("admin_debuff")) 
        {
            try 
            {
                ...
                if (activeChar.getTarget() instanceof L2PcInstance) 
                {
                    ...
                    if (sendMessage) 
                        player.sendPacket(SystemMessage.getSystemMessage(SystemMessageId.EFFECT_S1_DISAPPEARED).addSkillName(skillId));
                    else
                    {
                        L2Skill debuff = SkillTable.getInstance().getInfo(skillId, 1);
                        ArrayList<L2Effect> effects = new ArrayList<>(debuff.getEffectTemplates().length);
