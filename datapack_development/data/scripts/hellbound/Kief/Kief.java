...
public class Kief extends Quest
{
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		...
		{
			long stinger = st.getQuestItemsCount(10012);
			if (stinger >= 20)
			{
		...
		{
			long dimlf = st.getQuestItemsCount(9680);
			if (dimlf >= 1)
			{
		...
		{
			long lifef = st.getQuestItemsCount(9681);
			if (lifef >= 1)
			{
		...
		{
			long conlf = st.getQuestItemsCount(9682);
			if (conlf >= 1)
			{
